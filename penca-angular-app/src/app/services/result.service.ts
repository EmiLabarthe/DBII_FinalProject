...
export class ResultService {
  ...
  getResults(): Observable<IMatchResult[]> {
    return this.http.get<IMatchResult[]>(this.resultsUrl).pipe(
  ...
  getResult(id: bigint): Observable<IMatchResult> {
    ...
    return this.http.get<IMatchResult>(url).pipe(
   ...
   */
  getPredictionResultItems(
    studentId: string
  ): Observable<IPredictionResultItem[]> {
    const url = `${this.resultsUrl}/${studentId}-prediction`;
    return this.http.get<IPredictionResultItem[]>(url).pipe(
      tap((_) => console.log(`fetched student '${studentId}' result items`)),
      catchError(
        this.handleError<IPredictionResultItem[]>('getPredictionResults', [])
      )
    );
  }
  ...
  getPoints(
    localGoals: number,
    localPredictedGoals: number,
    visitorGoals: number,
    visitorPredictedGoals: number
  ): number {
    // predicted the exact result correctly
    if (
      localGoals == localPredictedGoals &&
      visitorGoals == visitorPredictedGoals
    ) {
      return 4;
    } // predicted the winner correctly
    else if (
      (localGoals > visitorGoals &&
        localPredictedGoals > visitorPredictedGoals) ||
  ...
  addResult(
    ...
    return this.http
      ...
      .pipe(catchError(this.handleError<IMatchResult>('add')));
  }
